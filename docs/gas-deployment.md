...
| --------------------- | -------------------- | ------------------------------------------------------------ |
| `AUTH_SPREADSHEET_ID` | Your Google Sheet ID | ID of the spreadsheet for storing users                      |
| `APP_NAME`            | Your app name        | Name of app that will be used by emails etc                  |
| `ENCRYPTION_KEY`      | Random secret key    | Used for token encryption (generate a random 32-char string) |
| `TOKEN_TTL_MINUTES`   | `15`                 | Auth token time-to-live (15 mins)                            |
| `OTP_TTL_HOURS`       | `2`                  | OTP time-to-live for password reset                          |
| `APP_TIMEZONE`        | `Africa/Douala`      | Timezone for timestamps                                      |

**How to get IDs:**

- **AUTH_SPREADSHEET_ID**: Create a new Google Sheet, copy the ID from the URL: `https://docs.google.com/spreadsheets/d/AUTH_SPREADSHEET_ID_HERE/edit`
- **ENCRYPTION_KEY**: Generate a random string (e.g., `openssl rand -hex 32` on Mac/Linux)

### Step 4: Create Required Sheets

1. Open the Google Spreadsheet (using the AUTH_SPREADSHEET_ID)
2. Create sheet:
...
- **NB** When sending requests to GAS, set your content-type to text (seems like google want to sabotage)
...
1. **Login to production account**:
   ...
   ```bash
   clasp login
   # Login with: (client account email) notaire.danielbechem7@gmail.com (for client/bechem branch)
   ```
