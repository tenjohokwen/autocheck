...
const SecurityInterceptor = {
  ...
  validateRequest: function (e) {
    ...
    if (this.isAdminRoute(action) && !RoleValidator.isAdmin(user)) {
  ...
  extractToken: function (e, requestData) {
    ...
    throw ResponseHandler.unauthorizedError('Authentication token required', 'error.token.required')
  },
  ...
  /**
   * @deprecated Use isAdminRoute instead
   * Kept for backward compatibility
   */
  isFleetManagerRoute: function (action) {
    return this.isAdminRoute(action)
  },
